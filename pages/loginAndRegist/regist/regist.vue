<template>
	<view class="content">
		...
		<!-- 注册密码 -->
		<view class="passWrap">
			<input :type="passType" :value="password" @input="onPassword" placeholder="请输入密码" placeholder-class="place" class="password"/>
			<image :src="isSee" class="password_isApprence" @tap="ImageChange"></image>
		</view>
		<!-- 注册按钮 -->
		<button class="regist" :disabled="regist_btn" @tap="goRegist">注册</button>
		<!-- 协议 -->
		...
		methods: {
			...
			onCode:function(e){
				...
				if(this.phone!=""&&this.code!=""&&this.password!=""){
					this.regist_btn=false;
				}else{
					this.regist_btn=true;
				};
			},
			// 获取密码
